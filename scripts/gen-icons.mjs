// Generates PWA icons with proper transparent background and safe-zone padding
// viewBox padded to -100/-100/200/200 so the spiral (extends to ~±78 SVG units)
// fits inside the 80% maskable safe zone (±80 units on a 200-unit canvas).
...
for (const size of [192, 512]) {
  ...
  await sharp(svgBuf, { density: Math.round(size * 72 / 200) })
    .resize(size, size)
    .png()
    .toFile(dest)
...
// apple-touch-icon: 180x180 with dark background (iOS fills transparent with white)
await sharp(svgBuf, { density: Math.round(180 * 72 / 200) })
  .resize(180, 180)
...
// favicon-96x96: transparent
await sharp(svgBuf, { density: Math.round(96 * 72 / 200) })
  .resize(96, 96)
  .png()
